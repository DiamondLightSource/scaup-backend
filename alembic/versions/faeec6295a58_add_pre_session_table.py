...
def upgrade() -> None:
    ...
    op.create_table('PreSession',
    sa.Column('preSessionId', sa.Integer(), nullable=False),
    sa.Column('shipmentId', sa.Integer(), nullable=False),
    sa.Column('details', sa.JSON(), nullable=True, comment='Generic additional details'),
    sa.ForeignKeyConstraint(['shipmentId'], ['Shipment.shipmentId'], ),
    sa.PrimaryKeyConstraint('preSessionId')
    )
    op.create_index(op.f('ix_PreSession_preSessionId'), 'PreSession', ['preSessionId'], unique=False)
    op.create_index(op.f('ix_PreSession_shipmentId'), 'PreSession', ['shipmentId'], unique=True)
